...
// There 2 types of object binding in JS
// Implicit binding which we do using (.) dot operator like Calc.add(5);
// In Implicit Binding, this keyword refer to current object
// Othere type is Explicit Binding, which we do using call,bind and apply
// this keyword is used to refer something
// if we use this in global context
console.log(this); // points to window object

// Let say we put it inside a function
this.a = 5; // global
function getParams() {
  console.log(this.a); // 5
}

getParams(); // this will target to window object only
...
let user = {
  ...
  age: 21,
  getDetails() {
    // Here inside normal function, this will refer to its parent object, not the window object so we print 'loki'
    // if there is no property named 'name' inside object user, then this.name will print undefined
    console.log(this.name); // loki
    console.log(this); // Object { name: "loki", age: 21, getDetails: getDetails() }
  },
};
...
const User = new user4("lokesh");
console.log(User); // Object { name: "lokesh" }
...
const user5 = {
  firstname: "Lokesh!",
  getName() {
    // this will point to user5 object so Lokesh! will be printed
    const firstname = "Lokesh Melkani";
    return this.firstname;
  },
};

console.log(user5.getName()); // Lokesh!
